...
/// TODO: split up into `FilterError`.
...
pub enum Error {
    ...
    #[error("The chosen `tick_size` of the quantity filter does not work with the chosen `min_quantity`. `min_quantity` must be a multiple of `step_size`")]
    InvalidMinQuantity,
