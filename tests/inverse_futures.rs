...
fn inv_long_market_win_full() {
    ...
    let _ = exchange.update_state(1, bba!(2000.0, 2000.0));
...
fn inv_long_market_loss_full() {
    ...
    let _ = exchange.update_state(2, bba!(800.0, 800.0));
...
fn inv_short_market_win_full() {
    ...
    let _ = exchange.update_state(1, bba!(800.0, 800.0));
    assert_eq!(exchange.account().position().unrealized_pnl(), 0.2);
...
fn inv_short_market_loss_full() {
    ...
    let _ = exchange.update_state(3, bba!(2000.0, 2000.0));
...
fn inv_short_market_win_partial() {
    ...
    exchange.submit_order(o).unwrap();
    let _ = exchange.update_state(3, bba!(800.0, 800.0));
...
fn inv_test_market_roundtrip() {
    ...
    let _ = exchange.update_state(2, bba!(1000.0, 1000.0));
    ...
    let buy_order = Order::market(Side::Buy, size).unwrap();
    exchange.submit_order(buy_order).unwrap();
    let _ = exchange.update_state(3, bba!(1000.0, 1000.0));
...
fn inv_execute_limit() {
    ...
    let (exec_orders, liq) = exchange.update_state(1, bba!(750.0, 750.0));
    assert!(!liq);
    ...
    let _ = exchange.update_state(1, bba!(1200.0, 1200.0));
    ...
    let _ = exchange.update_state(2, bba!(1200.1, 1200.1));
