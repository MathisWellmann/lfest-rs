...
fn inv_short_market_win_full() {
    ...
    let bid = quote!(1000);
    assert!(exchange
        .update_state(0, bba!(bid, quote!(1001)))
        .unwrap()
        .is_empty());
    ...
    assert_eq!(
        exchange.user_balances(),
        UserBalances {
            available_wallet_balance: base!(0.2) - fee0,
            position_margin: base!(0.8),
            order_margin: base!(0)
        }
    );
...
fn inv_short_market_loss_full() {
    ...
    let bid = quote!(999);
    let ask = quote!(1000);
    assert!(exchange.update_state(1, bba!(bid, ask)).unwrap().is_empty());
...
fn inv_test_market_roundtrip() {
    ...
    let ask = quote!(1000);
    assert!(exchange
        .update_state(0, bba!(quote!(999), ask))
        .unwrap()
        .is_empty());
